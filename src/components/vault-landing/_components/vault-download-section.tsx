...
import { ArrowRight, Download, ExternalLink, Github, Monitor, TestTubeDiagonal } from "lucide-react";
...
export function VaultDownloadSection({ downloadData, msStoreUrl, isDevMode }: VaultDownloadSectionProps) {
  ...
  return (
    <section id="download" className="mx-auto max-w-7xl scroll-mt-24 px-4 py-20 sm:px-6 lg:px-8">
      <motion.div
      ...
      >
        ...
        <h2 className="font-display text-4xl md:text-6xl">Instalacao pronta para evoluir com releases</h2>
        <p className="max-w-3xl text-lg text-text-secondary">
          A landing consulta a API do GitHub para listar versoes. Enquanto nao houver instaladores publicados, o
          download principal permanece pela Microsoft Store.
        </p>
      </motion.div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <BorderGradientPanel>
          <div className="space-y-5 px-6 py-6">
            <Badge className="bg-primary-crx text-on-primary-crx">Canal oficial atual</Badge>
            <h3 className="font-display text-3xl">Microsoft Store</h3>
            <p className="text-text-secondary">
              O canal prioritario para download do Critix Vault no momento. O link oficial definitivo pode ser
              atualizado sem alterar a estrutura da pagina.
            </p>
            <Button
              asChild
              size="lg"
        ...
        <BorderGradientPanel>
          <div className="space-y-5 px-6 py-6">
            ...
            <Button asChild variant="outline" className="rounded-full border-white/20 bg-black/20 hover:bg-black/35">
              <a href={`${downloadData.repositoryUrl}/releases`} target="_blank" rel="noreferrer">
      ...
      {hasInstallableAssets ? (
              ...
              <Card className="border-white/10 bg-bg-surface/85">
                <CardHeader>
                  <CardTitle className="flex items-center justify-between gap-3 text-lg">
                    ...
                    <Badge variant="outline" className="border-primary-crx/30 bg-primary-crx/10 text-primary-crx">
                      {asset.releaseTag}
                    </Badge>
                  </CardTitle>
                  <CardDescription className="line-clamp-2 text-text-secondary">{asset.name}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <p className="text-sm text-text-secondary">Tamanho: {asset.sizeLabel}</p>
                  <Button
                    asChild
        ...
        <BorderGradientPanel className="mt-8">
          <div className="space-y-5 px-6 py-6">
            <p className="text-text-muted text-sm uppercase tracking-[0.18em]">Estado atual do repositorio</p>
            <h3 className="font-display text-3xl">Sem assets de instalacao em releases</h3>
            <p className="max-w-3xl text-text-secondary">
              A lista de versoes abaixo e carregada pela API do GitHub. Quando houver release com instalador para
              desktop, os botoes de download direto serao habilitados automaticamente nesta secao.
            </p>

            <div className="grid gap-3 md:grid-cols-2">
              {downloadData.releases.length > 0
                ? downloadData.releases.map((release) => (
                    <Card key={release.id} className="border-white/10 bg-black/25">
                      <CardHeader className="pb-4">
                        <CardTitle className="text-lg">{release.tagName}</CardTitle>
